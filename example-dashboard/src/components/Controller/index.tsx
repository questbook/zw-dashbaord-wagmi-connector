...
export default function SmartViewer({ updateProjects }: Props) {
    ...
    const { selectedEntity, doesScwExist } =
    ...
    if ('createNew' in selectedEntity) {
        if (selectedEntity.createNew === 'project') return <ProjectCreator onCreate={updateProjects} />
        if (selectedEntity.createNew === 'gasTank') return <GasTankCreator projectId={selectedEntity.projectId} onCreate={updateProjects} />
    }
